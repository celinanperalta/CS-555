...
def test_birth_before_marriage(capfd):
    obj = Family(id="F01", husband="", wife="", birth = datetime.datetime.now() + datetime.timedelta(days=1), marriage_date=datetime.datetime.now())
    check_US02(obj)
...
def test_valid_marriage(capfd):
    obj = Family(id="FO1", husband="", wife="", marriage_date=datetime.datetime.now(), birth = datetime.datetime.now() + datetime.timedelta(days=1))
    check_US02(obj)
    out, err = capfd.readouterr()
    assert out.strip() == ""
...
def test_marriage_no_birth(capfd):
    obj = Family(id="F01", husband="", wife="", marriage_date=datetime.datetime.now())
    check_US02(obj)
    out, err = capfd.readouterr()
